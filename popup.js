...
document.addEventListener("DOMContentLoaded", evt => {
	...
	// Gets the current state of the extension (whether it's turned off or on).
	// var currentState = document.getElementById("extension-toggle").className;	
...
function switchState(button) {
	if (button.className == "dw-filter-on") { // If already on, turn off.
		...
		//chrome.storage.sync.set({ "extension-state": "dw-filter-on" });
		//var status = chrome.storage.sync.get(["extension-state"]);
		//console.log(status);
	}
}
